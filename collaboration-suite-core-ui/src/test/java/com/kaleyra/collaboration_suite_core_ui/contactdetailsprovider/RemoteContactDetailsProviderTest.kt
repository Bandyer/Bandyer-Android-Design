...
class RemoteContactDetailsProviderTest {
    ...
    fun `test contacts details timed out when contact have null name and image`() = runTest(testDispatcher) {
        val contacts = RemoteContactDetailsProviderTestHelper.ContactsMock(hashMapOf("userId1" to RemoteContactDetailsProviderTestHelper.ContactMock("userId1")))
    ...
    fun `test contacts details delayed fetch`() = runTest(testDispatcher) {
        val timeout = 1500L
        val contact1 = RemoteContactDetailsProviderTestHelper.ContactMock(
            "userId1",
            MutableStateFlow<String?>(null).emitWithDelay(
                newValue = "username1",
                delay = timeout,
                backgroundScope
            ),
            MutableStateFlow<Uri?>(null).emitWithDelay(
                newValue = uriUser1,
                delay = timeout,
                backgroundScope
            )
        )
        val contact2 = RemoteContactDetailsProviderTestHelper.ContactMock(
            "userId2",
            MutableStateFlow<String?>(null).emitWithDelay(
                newValue = "username2",
                delay = timeout,
