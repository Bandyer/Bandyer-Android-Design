...
internal class LocalContactDetailsProviderTest {

    private val testDispatcher = StandardTestDispatcher()

    private val uriUser1 = mockk<Uri>()
    ...
    fun `test contacts details empty user ids`() = runTest(testDispatcher) {
        val provider = LocalContactDetailsProvider(
            usersDescription = getUsersDescription(),
    ...
    fun `test contacts details immediate fetch`() = runTest(testDispatcher) {
        val provider = LocalContactDetailsProvider(
            usersDescription = getUsersDescription(fetchDelay = 0L),
            ioDispatcher = testDispatcher
        )
    ...
    fun `test contacts details delayed fetch`() = runTest(testDispatcher) {
        val provider = LocalContactDetailsProvider(
            usersDescription = getUsersDescription(fetchDelay = 1500L),
            ioDispatcher = testDispatcher
        )
        val result = provider.fetchContactsDetails("userId1", "userId2")
        assertEquals(successResult, result)
    ...
    fun `test contacts details fetch timed out`() = runTest(testDispatcher) {
        val timeout = 1000L
        val provider = LocalContactDetailsProvider(
            usersDescription = getUsersDescription(fetchDelay = timeout),
            ioDispatcher = testDispatcher
        )
    ...
    fun `test contacts details fetch timeout limit`() = runTest(testDispatcher) {
        val timeout = 1000L
        val provider = LocalContactDetailsProvider(
            usersDescription = getUsersDescription(fetchDelay = timeout - 1),
            ioDispatcher = testDispatcher
        )
        val result = provider.fetchContactsDetails("userId1", "userId2", timeout = timeout)
        assertEquals(successResult, result)
    ...
    private fun getUsersDescription(fetchDelay: Long = 0L): UsersDescription {
        return DefaultUsersDescription(
            name = { userIds ->
                delay(fetchDelay)
                when {
                    userIds.find { it == "userId1" } != null -> "username1"
                    userIds.find { it == "userId2" } != null -> "username2"
                    else -> "null"
                }
            },
            image = { userIds ->
                delay(fetchDelay)
                when {
                    userIds.find { it == "userId1" } != null -> uriUser1
                    userIds.find { it == "userId2" } != null -> uriUser2
                    else -> Uri.EMPTY
                }
            }
        )
