...
class GlassGestureDetector(context: Context?, onGestureListener: OnGestureListener) {
    ...
    val tapAndHoldCountDownTimer: CountDownTimer = object : CountDownTimer(TAP_AND_HOLD_THRESHOLD_MS.toLong(),
    ...
    private fun detectGesture(): Boolean {
        ...
        val tan = if (firstFingerDistanceX != 0f) Math.abs(firstFingerDistanceY / firstFingerDistanceX).toDouble() else Double.MAX_VALUE
        return if (isTwoFingerGesture) {
            val tanSecondFinger = if (secondFingerDistanceX != 0f) Math.abs(secondFingerDistanceY / secondFingerDistanceX).toDouble() else Double.MAX_VALUE
