 ...
 * @constructor
 */
class GoogleGlassGestureDetector(context: WeakReference<Context>, onGestureListener: OnGestureListener) {
    /**
    ...
    private fun detectGesture(): Boolean {
        ...
        val tan = (if (firstFingerDistanceX != 0f) abs(firstFingerDistanceY / firstFingerDistanceX).toDouble() else Double.MAX_VALUE).toDouble()
        return if (isTwoFingerGesture) {
            val tanSecondFinger = (if (secondFingerDistanceX != 0f) abs(secondFingerDistanceY / secondFingerDistanceX).toDouble() else Double.MAX_VALUE).toDouble()
    ...
    init {
        val touchSlop = ViewConfiguration.get(context.get()).scaledTouchSlop
