...
internal object ChatActionsMapper {
    fun Set<ChatUI.Action>.mapToChatActions(call: (Call.PreferredType, Set<CallUI.Action>) -> Unit): Set<ChatAction> {
        return mutableSetOf<ChatAction>().apply {
            ...
            actions.firstOrNull { !it.preferredType.hasVideo() }?.also { action ->
                add(ChatAction.AudioCall { call(action.preferredType, action.callActions) })
            }
            actions.firstOrNull { !it.preferredType.isVideoEnabled() }?.also { action ->
                add(ChatAction.AudioUpgradableCall { call(action.preferredType, action.callActions) })
            }
            actions.firstOrNull { it.preferredType.isVideoEnabled() }?.also { action ->
                add(ChatAction.VideoCall { call(action.preferredType, action.callActions) })
            }
        }
    }
}